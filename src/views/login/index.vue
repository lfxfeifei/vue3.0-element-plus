<template>
   <div class="login-container" ref="ll">
...
export default defineComponent ({
  ...
  setup(props: any, context: any ) {
    ...
    const handleLogin = async () => {
      let el:any = loginFormRef.value
      try {
        let flag = el.validate()
        if (flag) {
          data.loading = true
          console.log(context, 'context')
          store.dispatch('user/LoginByUsername', data.loginForm)
          data.loading = false
      ...
      //     .then(() => {
      //       data.loading = false
      //       data.$router.push({ path: data.redirect || '/' })
      //     }).catch(() => {
      //       data.loading = false
      //     })
      //   } else {
    ...
    const afterQRScan = () => {
      ...
      //   tencent: 'code'
      // }
      // const codeName = hashObj[codeMap[data.auth_type]]
      // if (!codeName) {
      //   alert('第三方登录失败')
      // } else {
      //   data.$store.dispatch('LoginByThirdparty', codeName).then(() => {
